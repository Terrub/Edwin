...
local function listedAuctionsUpdateHandler()
    ...
    if edwin_is_scanning and scanned_pages[current_page] == PAGE_READY_FOR_SCANNING then
        ...
        -- http://vanilla-wow.wikia.com/wiki/API_GetNumAuctionItems
        auctions_this_page, auctions_total = GetNumAuctionItems("list")

        -- NUM_AUCTION_ITEMS_PER_PAGE is actually a client Constant!
        amount_of_pages = ceil(auctions_total / NUM_AUCTION_ITEMS_PER_PAGE)
...
local function resetScannedResults()
    ...
    auctionFrameList:SetVerticalScroll(current_scroll_position)
    auctionFrameList:UpdateScrollChildRect()
...
local function scanListedAuctions(proposed_item_name)
    ...
    if proposed_item_name ~= EMPTY_PAGE_SEARCH_NAME then

        report("Starting search for", proposed_item_name)
...
local function newOwnAuctionUpdateHandler()
    ...
    -- http://vanilla-wow.wikia.com/wiki/API_GetAuctionSellItemInfo
    name, _,
    auctioning_item_count, _, _,
    auctioning_item_vendor_price = GetAuctionSellItemInfo()

    if not name then

        name = EMPTY_PAGE_SEARCH_NAME
...
local function createAuctionFrameList()

    auctionFrameList = CreateFrame("ScrollFrame", nil, base)

    auctionFrameList:SetWidth(
        auctionFrameHeader:GetWidth()
    )
    auctionFrameList:SetHeight(
        base:GetHeight() - auctionFrameHeader:GetHeight() - 1
    )

    auctionFrameList:SetPoint("TOPLEFT", auctionFrameHeader, "BOTTOMLEFT", 0, -1)

    auctionFrameList:EnableMouseWheel(true)
    auctionFrameList:SetScript("OnMouseWheel", function()

        if arg1 == -1 then

            current_scroll_position = min(
                (current_scroll_position + 30),
                auctionFrameList:GetVerticalScrollRange()
            )
        ...
        elseif arg1 == 1 then
            ...
            throwError("unrecognised arg1 for auctionFrameList::OnMouseWheel")
...
local function createAuctionFrameListItems()
    ...
    auctionFrameListItems = CreateFrame("Frame", nil, auctionFrameList)

    auctionFrameListItems:SetWidth(auctionFrameList:GetWidth())
    auctionFrameListItems:SetHeight(auctionFrameList:GetHeight())
...
local function constructAddon()
    ...
    createAuctionFrameList()
    createAuctionFrameListItems()

    auctionFrameList:SetScrollChild(auctionFrameListItems)
...
local function populateSlashCommandList()
    ...
    addSlashCommand(
        "stop",
        stopScanningProces,
        '<|cff9999ffaction|r> |cff999999-- Attempts to gracefully stop a scan in progress.|r'
    )
